    ...
    <div class="flex max-lg:flex-col gap-6">
        ...
        <div class="flex flex-col gap-6 w-full lg:w-1/2">
            ...
            <div class="flex flex-col gap-2 w-full">
                <div class="flex items-center gap-2" v-for="(value, key) in data[0].features">
                    <p class="first-letter:capitalize">{{ key }}</p>
                    <p class="min-w-[50%] grow h-px border-b border-white/15 border-dashed"></p>
                    <p class="first-letter:capitalize">{{ value }}</p>
                </div>
            </div>
            ...
            <button type="button" class="px-4 py-0.5 rounded-full transition-all duration-500 hover:bg-white/5 h-8 text-[#0B0D17] text-center w-[260px] font-Cormorant uppercase relative overflow-hidden group">
